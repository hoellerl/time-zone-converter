...
function convertTime(){
  ...
  while (timezone === undefined){
      let tz = prompt("No timezone found in selected text, please enter it manually! (abbreviation or +/-HH:MM)");
      if (Object.keys(timezones).includes(tz)){
          timezone = timezones[tz];
      }
      else if (timezoneFormat.test(tz)){
          ...
          timezone = "Z";
          console.log(`DTC: Invalid or unsupported timezone format (${tz}), defaulting to UTC! If you are sure it is valid, please open an issue on GitHub.`);
      }
  }
  let iso;
...
function replaceText(dateString) {
